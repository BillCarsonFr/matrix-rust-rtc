...
pub(crate) fn classify(event: &RawMatrixEvent, config: &SessionConfig, now: u64) -> Ingest {
    ...
    match event_type {
        ...
        ROOM_ENCRYPTION_EVENT_TYPE => {
            let has_algorithm = content(event)
...
fn classify_member(event: &RawMatrixEvent, now: u64) -> Ingest {
    ...
    match conversion {
        ...
        Msc4143Conversion::Removal { sticky_key } => Ingest::MemberRemoval {
            key: key(sticky_key),
            event_id,
            expires_at,
        },
...
mod tests {
    ...
    fn classifies_both_member_and_slot_spellings() {
        ...
        for t in SLOT_EVENT_TYPES {
            ...
            ev["type"] = json!(t);
            assert!(
                matches!(classify_now(ev, ElementCallCompat::Off), Ingest::Slot { slot_id, .. } if slot_id == SLOT_ID)
            );
        }
    }
    ...
    fn classifies_room_member_and_encryption() {
        ...
        for membership in ["leave", "ban", "invite"] {
            assert_eq!(
                classify_now(
                    room_member_event("@a:x", membership, NOW),
                    ElementCallCompat::Off
                ),
                Ingest::RoomMember {
                    user_id: "@a:x".into(),
        ...
        assert_eq!(
            classify_now(room_encryption_event(NOW), ElementCallCompat::Off),
            Ingest::RoomEncryption
        );
        ...
        empty["content"] = json!({});
        assert!(matches!(
            classify_now(empty, ElementCallCompat::Off),
            Ingest::Ignored(_)
        ));
    }
    ...
    fn msc3401_is_ignored_outside_state_events_compat() {
        ...
        assert!(matches!(
            classify_now(ev.clone(), ElementCallCompat::Off),
            Ingest::Ignored(_)
        ));
        assert!(matches!(
    ...
    fn unknown_and_malformed_events_are_ignored_not_errors() {
        assert!(matches!(
            classify_now(
                json!({ "type": "m.room.message", "sender": "@a:x", "content": {} }),
                ElementCallCompat::Off
            ),
            Ingest::Ignored(_)
        ));
        ...
        assert!(matches!(
            classify_now(
                json!({ "type": "m.rtc.member", "content": {} }),
                ElementCallCompat::Off
            ),
            Ingest::Ignored(_)
        ));
        assert!(matches!(
            classify_now(
                json!({ "type": "m.rtc.member", "sender": "@a:x" }),
                ElementCallCompat::Off
            ),
            Ingest::Ignored(_)
        ));
        assert!(matches!(
            classify_now(
                json!({ "type": "m.rtc.member", "sender": "@a:x", "content": 5 }),
                ElementCallCompat::Off
            ),
            Ingest::Ignored(_)
        ));
        assert!(matches!(
            classify_now(json!("not even an object"), ElementCallCompat::Off),
            Ingest::Ignored(_)
        ));
    }
    ...
    fn a_mock_driver_shaped_join_parses() {
        ...
        match classify_now(event, ElementCallCompat::Off) {
            Ingest::Member { candidate, .. } => {
                ...
                assert_eq!(
                    candidate.member.transports.published[0].properties["livekit_service_url"],
                    LK_SERVICE_URL
                );
            }
            other => panic!("{other:?}"),
