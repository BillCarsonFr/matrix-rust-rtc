...
describe("encryption", () => {
  ...
  it("manageMediaKeys decides when the slot is unknown, and the slot decides when it is known", async () => {
    ...
    expect(unknown.manager.session().failedReads).toContain(FfiSessionRead.Slot);
    await new Promise((r) => setTimeout(r, 50));
    expect(unknown.driver.calls("toDevice")).toEqual([]);
    ...
    await new Promise((r) => setTimeout(r, 50));
    expect(plain.driver.calls("toDevice")).toEqual([]);

    async function manager_join(m: ReturnType<typeof newManager>["manager"]) {
      await m.join(receiveOnly(), joinParams);
    }
  });
